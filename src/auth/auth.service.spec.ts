...
describe('AuthService', () => {
  ...
  describe('loginUser', () => {
    it('should login a user with valid credentials', async () => {
      ...
      expect(mockJwtTokenService.getAccessToken).toHaveBeenCalledWith(existingUser.id);
      expect(mockJwtTokenService.getRefreshToken).toHaveBeenCalledWith(existingUser.id);
    ...
    describe('registerUser', () => {
      it('should register a new user', async () => {
        const userDto: CreateUserDto = {
          firstName:'john', lastName:'calos', country:'sri lanka', region:'colombo', email: 'test@example.com', password: 'password'
        };
        ...
        expect(mockUserModel.save).toHaveBeenCalledWith(expect.objectContaining(userDto));
        expect(result).toEqual(userDto);
      });
  
      it('should throw an error if user already exists', async () => {
        const userDto: CreateUserDto = { firstName:'john', lastName:'calos', country:'sri lanka', region:'colombo', email: 'test@example.com', password: 'password' };
    ...
    describe('sendRegistrationMail', () => {
      it('should send a registration email', async () => {
        ...
        expect(mockTokenService.revokeAllActiveSignUpTokens).toHaveBeenCalledWith(email);
    ...
    describe('authorizeUser', () => {
      it('should authorize a user', async () => {
        ...
        expect(mockTokenService.claimToken).toHaveBeenCalledWith(tokenCode, TokenPurpose.SIGN_UP);
        expect(mockUserModel.findOne).toHaveBeenCalledWith({ email });
        expect(mockUserModel.save).toHaveBeenCalledWith(existingUser);
        expect(result).toEqual(expect.objectContaining({ email, isAuthorized: true }));
      });
  
      it('should throw an error if user not found', async () => {
        const tokenCode = 'token-code';
        mockTokenService.claimToken.mockReturnValue({ email: 'unknown@example.com' });
    ...
    describe('forgotUserPassword', () => {
      it('should send a forgot password email', async () => {
        ...
        mockUserModel.findOne.mockReturnValue(existingUser);
        mockTokenService.revokeAllActiveResetPasswordTokens.mockReturnValue(undefined);
  
        await authService.forgotUserPassword(email);
  
        expect(mockUserModel.findOne).toHaveBeenCalledWith({ email });
        expect(mockTokenService.revokeAllActiveResetPasswordTokens).toHaveBeenCalledWith(email);
      ...
      it('should not send an email if user not found', async () => {
        ...
        await expect(authService.forgotUserPassword(email)).resolves.not.toThrow();
    ...
    describe('resetUserPassword', () => {
      it('should reset a user password', async () => {
        ...
        expect(mockTokenService.claimToken).toHaveBeenCalledWith(tokenCode, TokenPurpose.RESET_PASSWORD);
        expect(mockUserModel.findOne).toHaveBeenCalledWith({ email });
        expect(mockHash).toHaveBeenCalledWith(password, 10);
        expect(mockUserModel.save).toHaveBeenCalledWith(expect.objectContaining({ password: 'hashedPassword' }));
      });
      ...
      it('should throw an error if user not found', async () => {
        ...
        await expect(authService.resetUserPassword(password, tokenCode)).rejects.toThrow(
          new BadRequestException(ErrorMessage.USER_NOT_FOUND),
        );
      });
    });
    ...
    describe('changeUserPassword', () => {
      it('should change a user password with valid old password', async () => {
        ...
        expect(mockHash).toHaveBeenCalledWith(password, 10);
        expect(mockUserModel.save).toHaveBeenCalledWith(expect.objectContaining({ password: 'hashedNewPassword' }));
      });
      ...
      it('should throw an error for invalid old password', async () => {
        ...
        await expect(authService.changeUserPassword(email, password, oldPassword)).rejects.toThrow(
          new BadRequestException(ErrorMessage.INVALID_CREDENTIALS),
        );
      });
      ...
      it('should not throw an error if user not found', async () => {
        ...
        await expect(authService.changeUserPassword(email, password, oldPassword)).resolves.not.toThrow();
      });
    });
  });
